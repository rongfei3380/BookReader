...
- (void)setText:(NSString *)text {
    ...
    if (_selectedFont) {
        CGRect seletedBounds = [text boundingRectWithSize:CGSizeMake(MAXFLOAT, 0.0) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:_selectedFont} context:nil];
        if (seletedBounds.size.width > bounds.size.width) {
            _titleSize = seletedBounds.size;
        } else {
...
-(void)setBadgePoint:(BOOL)show {
    [self createBageBtnIfNeeded];
    if (show) {
