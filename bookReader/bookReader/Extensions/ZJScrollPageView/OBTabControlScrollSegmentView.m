//
//  OBTabControlScrollSegmentView.m
//  OnBed
//
//  Created by chengfei on 2017/6/17.
//  Copyright © 2017年 iQcong Inc. All rights reserved.
//

#import "OBTabControlScrollSegmentView.h"
#import "ZJTitleView.h"
#import "UIView+ZJFrame.h"

@interface OBTabControlScrollSegmentView ()<UIScrollViewDelegate> {
...
@implementation OBTabControlScrollSegmentView

static CGFloat const xGap = 0;
...
//    }
    for (NSString *title in self.titles) {
        ...
        titleView.selectedFont = self.segmentStyle.selectedTitleFont;
        titleView.font = self.segmentStyle.titleFont;
        titleView.label.textAlignment = NSTextAlignmentCenter;
        titleView.label.userInteractionEnabled = YES;
//        titleView.backgroundColor = LX_UIColorFromRGBAInHex(0xffffff, 0.4);
//        label.numberOfLines = 2.f;
        ...
        CGFloat maxWidth = 183/2.f;
        
        CGRect bounds = [title boundingRectWithSize:CGSizeMake(maxWidth, 0.0) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: titleView.label.font} context:nil];
        bounds.size.width = bounds.size.width +self.segmentStyle.titleFont.pointSize;
...
- (void)coverLayerCorner:(UIView *)coverLayer {
    UIBezierPath *maskPath = [UIBezierPath bezierPathWithRoundedRect:coverLayer.bounds byRoundingCorners:UIRectCornerTopLeft | UIRectCornerTopRight cornerRadii:CGSizeMake(self.segmentStyle.coverCornerRadius, self.segmentStyle.coverCornerRadius)];
    
    CAShapeLayer *maskLayer = [[CAShapeLayer alloc] init];
    maskLayer.frame = coverLayer.bounds;
    maskLayer.path = maskPath.CGPath;
    coverLayer.layer.mask = maskLayer;
}
...
- (void)setUpTitleViewsPosition {
    ...
    if (!self.segmentStyle.isScrollTitle) {// 标题不能滚动, 平分宽度
        ...
        NSInteger index = 0;
        for (ZJTitleView *label in self.titleViews) {
...
//            allTitlesWidth = allTitlesWidth + [self.titleWidths[i] floatValue];
//        }
//        if (self.titleWidths.count > 1) {
//            allTitlesWidth = allTitlesWidth +(self.titleWidths.count -1) *self.segmentStyle.titleMargin;
//        }
        
        float addedMargin = self.segmentStyle.titleMargin;
        
        NSInteger index = 0;
        float lastLableMaxX = addedMargin;
        
        for (ZJTitleView *label in self.titleViews) {
            titleW = [self.titleWidths[index] floatValue];
            titleX = lastLableMaxX;
            
            lastLableMaxX += (titleW+ self.segmentStyle.titleMargin);
            
            label.frame = CGRectMake(titleX, titleY, titleW, titleH);
            index++;
            [self coverLayerCorner:label];
    ...
    if (currentTitleView) {
        ...
        // 设置初始状态文字的颜色
        currentTitleView.label.textColor = self.segmentStyle.selectedTitleColor;
//        currentTitleView.label.font = self.segmentStyle.selectedTitleFont;
    }
...
- (void)setupScrollLineAndCover {
    ...
    if (self.coverLayer) {
        ...
        if (self.segmentStyle.isScrollTitle) {
            ...
            [self coverLayerCorner:self.coverLayer];
...
- (void)adjustUIWhenBtnOnClickWithAnimate:(BOOL)animated taped:(BOOL)taped {
    ...
    [UIView animateWithDuration:animatedTime animations:^{
        ...
        if (weakSelf.coverLayer) {
            ...
            [weakSelf coverLayerCorner:weakSelf.coverLayer];
        }
...
//        [self coverLayerCorner:coverView];
